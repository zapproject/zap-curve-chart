...
export class BondHistorySvgLineChart {
  ...
  constructor(private container: HTMLElement, options: ChartOptions = {}, lineOptions: LineOptions = {}, ctxOptions: CtxOptions = {}) {
    ...
    this.svg.setAttribute('height', (height + 42).toString());
  ...
  draw(values: any, dots = this.options.maxDots) {
    const data = this.getDataset(values, dots);
  ...
  private onMouseMove(e) {
    ...
    const coef = (this.maxX - 1) / rect.width;
    let dots = Math.round((e.clientX - rect.left) * coef);
    let price = (this.values[dots]) ? this.values[dots] : 'N/A';
  ...
  private getDataset(values: any, maxDots): any {
    let maxY = Math.max(...values);
    this.values = values;
    this.maxX = values.length;

    const height = this.options.height / 2;
    const reduced = reduce(this.maxX, maxDots);
    const coefY = maxY / height;
    const coefX = this.options.width / (this.maxX - 1);
    let polyline = [];
    const fill = [`${0},${height}`];
    polyline = [];

    reduced.forEach(x => {
      polyline.push(`${(x-1) * coefX},${height - (values[x-1] / coefY)}`);
      fill.push(`${(x-1) * coefX},${height - (values[x-1] / coefY)}`);
    });
