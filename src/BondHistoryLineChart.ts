...
export class BondHistoryLineChart {
  ...
  private getDataset(values: number[]): Array<{x: number; y: number}> {
  ...
  public draw(values: number[] = []) {
    ...
    dataset.forEach((data, index) => {
      pointPersonalization.pointRadius.push(this.lineOptions.pointRadius);
      pointPersonalization.pointBorderWidth.push(this.lineOptions.pointBorderWidth);
    });
    this.chartDatasets = [{
      label: '',
